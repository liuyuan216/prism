...
import parser.ast.ExpressionPoss;
...
public class ProbModelChecker extends NonProbModelChecker
{
	...
	public StateValues checkExpression(Expression expr) throws PrismException
	{
		...
		// Poss operator by liuyuan
		else if (expr instanceof ExpressionPoss) {
			res = checkExpressionPoss((ExpressionPoss) expr);
		}
	...
	//Poss operator added by liuyuan
	protected StateValues checkExpressionPoss(ExpressionPoss expr) throws PrismException
	{
		// Get info from Po operator
		OpRelOpBound opInfo = expr.getRelopBoundInfo(constantValues);

		// Check for trivial (i.e. stupid) cases
		if (opInfo.isTriviallyTrue()) {
			mainLog.printWarning("Checking for possibility " + opInfo.relOpBoundString() + " - formula trivially satisfies all states");
			JDD.Ref(reach);
			return new StateValuesMTBDD(reach, model);
		} else if (opInfo.isTriviallyFalse()) {
			mainLog.printWarning("Checking for possibility " + opInfo.relOpBoundString() + " - formula trivially satisfies no states");
			return new StateValuesMTBDD(JDD.Constant(0), model);
		}
		...
		boolean qual = opInfo.isQualitativePoss() && precomp && prob0 && prob1;
		StateValues posss = checkPossPathFormula(expr.getExpression(), qual);

		// Print out possibilities
		if (prism.getVerbose()) {
			mainLog.print("\nPossibilities (non-zero only) for all states:\n");
			posss.print(mainLog);
		}
		...
		// Otherwise, compare against bound to get set of satisfying states
			...
			JDDNode sol = posss.getBDDFromInterval(opInfo.getRelOp(), opInfo.getBound());
			// remove unreachable states from solution
			JDD.Ref(reach);
			sol = JDD.And(sol, reach);
			// free vector
	...
	protected StateValues checkPossPathFormula(Expression expr, boolean qual) throws PrismException
	{
		// Test whether this is a simple path formula (i.e. PoCTL)
		// and whether we want to use the corresponding algorithms
		boolean useSimplePathAlgo = expr.isSimplePathFormula();
